...
final class Varintboundaryteststests: XCTestCase {
    ...
    struct VarintBoundaryTests {
        ...
        func testVarintShiftOverflowProtection() throws {
        ...
        func testVarintMaxBytesLimit() throws {
        ...
        func testVarintExact10Bytes() throws {
            ...
            XCTAssert(data.count <= 10) // Varint max is 10 bytes
            // Verify round-trip
            let decoder = BlazeBinaryDecoder(data: data)
            let decoded = try decoder.decodeInt()
            XCTAssert(decoded == Int.max)
        }
        func testVarintContinuationBitPattern() throws {
            ...
            for value in testValues {
                ...
                for i in 0..<(data.count - 1) {
                    XCTAssert((data[i] & 0x80) != 0, "Byte \(i) should have continuation bit set for value \(value)")
                }
                // Last byte should not have continuation bit
                let lastByte = data[data.count - 1]
                XCTAssert((lastByte & 0x80) == 0, "Last byte should not have continuation bit for value \(value)")
            }
        }
        func testZigZagBoundaryValues() throws {
            ...
            for value in boundaries {
                ...
                let decoder = BlazeBinaryDecoder(data: data)
                let decoded = try decoder.decodeInt()
                XCTAssert(decoded == value, "ZigZag boundary test failed for \(value)")
            }
        }
        func testZigZagRoundTripAllValues() throws {
            ...
            for value in testRange {
                ...
                let decoder = BlazeBinaryDecoder(data: data)
                let decoded = try decoder.decodeInt()
                XCTAssert(decoded == value, "ZigZag round-trip failed for \(value)")
            }
        }
    }
}
