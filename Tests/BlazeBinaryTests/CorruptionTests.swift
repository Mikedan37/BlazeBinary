...
final class CorruptionTests: XCTestCase {
    ...
    func testDecoderRejectsTruncatedRecord() throws {
        ...
        for truncateAt in 1..<validData.count {
            ...
            do {
                ...
                XCTFail("Decoder should reject truncated record at position \(truncateAt)")
    ...
    func testDecoderRejectsInvalidVarint() throws {
        ...
        do {
            ...
            XCTFail("Decoder should reject invalid varint (too many bytes)")
    ...
    func testFrameDecoderRejectsMismatchedLength() throws {
        ...
        do {
            ...
            if let frame = result {
                XCTAssert(frame.count <= frameData.count, "Frame parser should not return more data than available")
            }
        } catch {
    ...
    func testFrameDecoderRejectsOversizedFrame() throws {
    ...
    func testDecoderRejectsInvalidUTF8() throws {
        ...
        do {
            _ = try decoder.decodeString()
            XCTFail("Decoder should reject invalid UTF-8 sequence")
        } catch BlazeBinaryError.decodeFailed {
            // Expected error
        } catch {
    ...
    func testDecoderRejectsNegativeLength() throws {
    ...
    func testDecoderHandlesZeroLengthFields() throws {
        // Zero-length string
        ...
        XCTAssert(emptyString == "", "Zero-length string should decode to empty string")
        ...
        XCTAssert(emptyData.isEmpty, "Zero-length data should decode to empty Data")
    }
    
    /// Tests that corrupted frame headers are rejected.
    func testFrameDecoderRejectsCorruptedHeader() throws {
    ...
    func testDecoderRejectsPartialVarint() throws {
        ...
        do {
            _ = try decoder.decodeInt()
            XCTFail("Decoder should reject partial varint encoding")
        } catch BlazeBinaryError.truncated {
            // Expected error
        } catch BlazeBinaryError.invalidVarint {
            // Also acceptable
    ...
    func testDecoderRejectsGarbageData() throws {
        ...
        do {
            ...
            if let frame = result {
                XCTAssert(frame.count <= garbage.count, "Frame parser should not return more data than available")
            }
        } catch {
            // Any error is acceptable for garbage data
        }
